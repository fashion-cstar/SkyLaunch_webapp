...
import { AVAX, BNB, DEV, ETHER, JSBI, MATIC, TokenAmount, ETHER_CURRENCIES } from '@skylaunch/sdk'
...
export function useStakingInfoTop(
  ...
  const apr = getAPR(stakingInfo.getHypotheticalRewardRate(stakingInfo.stakedAmount.toFixed(0) != "0" ? stakingInfo.stakedAmount : new TokenAmount(stakingInfo.stakedAmount.token, '10000000000000000000000') , stakingInfo.totalStakedAmount, stakingInfo.totalRewardRate, 60 * 60 * 24 * 356, 1), stakingInfo.stakedAmount.toFixed(0) != "0" ? stakingInfo.stakedAmount : new TokenAmount(stakingInfo.stakedAmount.token, '10000000000000000000000'));
